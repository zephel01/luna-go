...
Luna automatically saves session conversations to a buffer. Run `luna dream` to let a local Ollama model summarize the buffer into `context.md`. This memory is injected automatically on the next startup.
...
~/.luna-go/
  ...
  memory/
    <project-name>/
      context.md      ← distilled memory (auto-injected at startup)
...
luna memory status  # Show buffer entry count and size
luna memory clear   # Reset buffer and context.md
```
...
~/.luna-go/
  ...
  memory/
    <project>/
      context.md                # Project memory (generated by luna dream)
